...
class ProductSeeder {
  ...
  static Future<void> seedStoresAndProducts() async {
    ...
    final categories = catsSnapshot.docs
        .map((d) => CategoryModel.fromJson(d.data(), d.id))
        .toList();
    final subcategories = subsSnapshot.docs
        .map((d) => SubCategoryModel.fromJson(d.data(), d.id))
        .toList();
    ...
    for (int s = 0; s < 20; s++) {
      ...
      storeData['categories'] =
          []; // We will dynamically assign this based on products
    ...
    print(
      '✅ Successfully seeded ${stores.length} stores, ${subscriptions.length} subscriptions, and ${products.length} products! Every subcategory now has at least 1 product.',
    );
  }
}
