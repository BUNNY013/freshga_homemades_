...
class SearchProvider with ChangeNotifier {
  ...
  Future<void> saveRecentSearch(String query) async {
    ...
    _recentSearches.removeWhere(
      (q) => q.toLowerCase() == formattedQuery.toLowerCase(),
    );
  ...
  Future<void> _fetchSuggestions(String query) async {
    try {
      ...
      final String cleanQuery = isStoreSearch
      ...
      final results = await _searchService.getLiveSuggestions(
        cleanQuery,
      ...
      if (isStoreSearch) {
        ...
        if (results['products'] != null) {
          rawProducts = (results['products'] as List<dynamic>)
              .cast<ProductModel>();
        }
        final pausedStoreIdsInSuggestions = rawStores
            .where((s) => !s.isActive)
            .map((s) => s.id)
            .toSet();
        _productSuggestions = rawProducts
            .where((p) => !pausedStoreIdsInSuggestions.contains(p.storeId))
            .toList();
      }
    } catch (e) {
  ...
  Future<void> performFullSearch(String query) async {
    ...
    try {
      ...
      List<ProductModel> products = isStoreSearch
          ? []
          : futures[0] as List<ProductModel>;
      List<StoreModel> baselineStores = isStoreSearch
          ? futures[0] as List<StoreModel>
          : futures[1] as List<StoreModel>;
      ...
      final missingStoreIds = productStoreIds
          .difference(existingStoreIds)
          .toList();

      if (missingStoreIds.isNotEmpty) {
        final additionalStores = await _searchService.getStoresByIds(
          missingStoreIds,
        );
      ...
      final activeStoreIds = baselineStores
          .where((s) => s.isActive)
          .map((s) => s.id)
          .toSet();

      _searchResultsProducts = products
          .where((p) => activeStoreIds.contains(p.storeId))
          .toList();
