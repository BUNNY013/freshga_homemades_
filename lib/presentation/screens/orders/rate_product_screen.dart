    ...
    try {
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        for (var item in uniqueProducts) {
          ...
          if (rating > 0) {
            final docRef = FirebaseFirestore.instance.collection('products').doc(item.productId);
        ...
        for (var item in uniqueProducts) {
          final rating = _ratings[item.productId] ?? 0;
          final feedback = _feedbackControllers[item.productId]?.text.trim() ?? '';
          ...
          if (rating > 0) {
            ...
            if (pDoc != null && pDoc.exists) {
              ...
              final currentCount = (data['totalReviews'] ?? data['reviewsCount'] ?? 0).toInt();
              final currentRatingCounts = Map<String, dynamic>.from(data['ratingCounts'] ?? {});

              final newCount = currentCount + 1;
              final newRating = ((currentRating * currentCount) + rating) / newCount;
              currentRatingCounts[rating.toString()] = (currentRatingCounts[rating.toString()] ?? 0) + 1;
        ...
        // Mark order as rated
        transaction.update(
          FirebaseFirestore.instance.collection('orders').doc(widget.order.orderId),
          {'isRated': true},
        );
      });
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to submit ratings: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text("Thank you!", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24)),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: widget.order.items.take(2).map((item) => Padding(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () => Navigator.popUntil(context, (route) => route.isFirst || route.settings.name == '/orders'),
                ...
                ...(() {
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          ClipRRect(
                            ...
                            child: CachedNetworkImage(
                              ...
                              height: 72,
                              fit: BoxFit.cover,
                              placeholder: (context, url) => Container(color: Colors.grey.shade100, width: 72, height: 72),
                              errorWidget: (context, url, error) => Container(color: Colors.grey.shade100, width: 72, height: 72, child: const Icon(Icons.image, color: Colors.grey)),
                            ),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(item.variantLabel, style: TextStyle(color: Colors.grey.shade600, fontSize: 14)),
                              ],
                            ),
                          )
                        ],
                      ...
                      Text("Rate this product", style: TextStyle(color: Colors.grey.shade800, fontSize: 14, fontWeight: FontWeight.w500)),
                      const SizedBox(height: 12),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: List.generate(5, (index) {
                          final isSelected = index < (_ratings[item.productId] ?? 0);
                      ...
                      const SizedBox(height: 24),
                      Text("Write a short feedback (optional)", style: TextStyle(color: Colors.grey.shade700, fontSize: 13)),
                      const SizedBox(height: 8),
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: "Tell us what you liked about this product...",
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isLoading 
                          ...
                          : const Text("Submit Ratings", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  ),
                ),
              ],
