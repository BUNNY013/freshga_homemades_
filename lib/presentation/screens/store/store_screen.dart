  ...
  void _scrollToSection(String categoryId, String sectionName) async {
    ...
    for (var section in _sortedKeys) {
      ...
      if (!_collapsedSections.contains(section)) {
        targetOffset +=
            _groupedProducts[section]!.length * 224.0; // Fixed item height
      }
    }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
              ...
              return NestedScrollView(
                headerSliverBuilder: (context, innerBoxIsScrolled) {
                  return [
                    ...
                    SliverToBoxAdapter(child: StoreInfoSection(store: store)),
                    ...
                    children: [
                      ...
                      Builder(
                        builder: (innerContext) =>
                            _buildShopTab(innerContext, provider),
                      ),
          ...
          Consumer<StoreProvider>(
            builder: (context, provider, child) {
              if (provider.currentStore == null || provider.isLoadingProducts)
                return const SizedBox.shrink();
              if (_tabController.index != 0)
                return const SizedBox.shrink(); // Only show on Shop tab

              final Map<String, int> sectionCounts = {};
              for (var product in provider.filteredProducts) {
                if (product.subCategoryIds.isEmpty) {
                  sectionCounts['Other Delights'] =
                      (sectionCounts['Other Delights'] ?? 0) + 1;
                } else {
                  for (var subId in product.subCategoryIds) {
                    final subName =
                        provider.availableSubcategories[subId]?['name'] ??
                        "Other Delights";
          ...
          Consumer<StoreProvider>(
            builder: (context, provider, child) {
              if (!provider.isStoreActive ||
                  provider.currentStore?.isSuspended == true ||
                  provider.currentStore?.isActive == false) {
  ...
  Widget _buildShopTab(BuildContext context, StoreProvider provider) {
    ...
    if (provider.selectedCategory == "All") {
      final bestSellers = products
          .where((p) => p.isTrending || p.rating >= 4.5 || p.reviewsCount >= 10)
          .toList();
      ...
      if (offers.isNotEmpty) {
        _groupedProducts["Discounts"] = offers
            .toList(); // Show all discounted products
      }
    }
    ...
    _sortedKeys = _groupedProducts.keys.toList()
      ..sort((a, b) {
              ...
              return [
                ...
                if (!isCollapsed)
                  SliverFixedExtentList(
                    itemExtent:
                        224.0, // Pre-calculated exact height for perfect scrolling
                    delegate: SliverChildBuilderDelegate((context, index) {
                      return StoreProductListItem(
          ...
          SliverToBoxAdapter(
            child: Padding(
              padding: const EdgeInsets.only(
                top: 40,
                bottom: 120,
              ), // Ensures last item isn't blocked by FAB
              child: Column(
                children: [
                  Icon(
