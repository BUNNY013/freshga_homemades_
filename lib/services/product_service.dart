...
class ProductService {
  ...
  Future<List<ProductModel>> getRandomDiscoveryProducts({
    int limit = 50,
    String? customerState,
  }) async {
    try {
  ...
  Future<List<ProductModel>> getSimilarProducts(
    ...
    try {
      ...
      products.sort((a, b) {
        int aOverlap = a.subCategoryIds
            .where((sub) => product.subCategoryIds.contains(sub))
            .length;
        int bOverlap = b.subCategoryIds
            .where((sub) => product.subCategoryIds.contains(sub))
            .length;
        ...
        int aTagsOverlap = a.tags
            .where((tag) => product.tags.contains(tag))
            .length;
        int bTagsOverlap = b.tags
            .where((tag) => product.tags.contains(tag))
            .length;
